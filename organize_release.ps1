...
if (Test-Path "dist\RMS Setup 1.0.0.exe") {
    Move-Item -Path "dist\RMS Setup 1.0.0.exe" -Destination "$ReleaseDir\Offline Installer" -Force
}

# Move Online Installer (and artifacts)
if (Test-Path "dist\nsis-web\RMS Web Setup 1.0.0.exe") {
    Move-Item -Path "dist\nsis-web\RMS Web Setup 1.0.0.exe" -Destination "$ReleaseDir\Online Installer" -Force
}
if (Test-Path "dist\nsis-web\*.7z") {
